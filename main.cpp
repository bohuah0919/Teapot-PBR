...
const std::string MODEL_PATH = "models/teapot_obj/teapot_obj.obj";
const std::vector<std::string> TEXTURE_PATHS =
{ "models/teapot_textures/Base_color.jpg",
    "models/teapot_textures/normal.jpg",
    "models/teapot_textures/roughness.jpg"
};
...
struct Vertex {
    ...
    bool operator==(const Vertex& other) const {
        return pos == other.pos && color == other.color && normal == other.normal && texCoord == other.texCoord && texIdx == other.texIdx;
    }
};
...
private:
    ...
    glm::vec3 orignal_focal = { 0.0f, 5.0f, 0.0f };
    glm::vec3 focal = orignal_focal;
    ...
    std::vector <VkCommandPool> shadowMapCommandPool;
    ...
    std::vector<VkImage> shadowMapColorImage;
    std::vector<VkDeviceMemory> shadowMapColorImageMemory;
    std::vector<VkImageView> shadowMapColorImageView;

    std::vector<VkImage> shadowMapDepthImage;
    std::vector<VkDeviceMemory> shadowMapDepthImageMemory;
    std::vector<VkImageView> shadowMapDepthImageView;
    ...
    std::vector<VkDescriptorPool> lightDescriptorPool;
    ...
    static void cursor_position_callback(GLFWwindow* window, double xpos, double ypos)
    {
        ...
        if (state == GLFW_PRESS) {
            ...
            myEngine->horizatal_angle_offset = (myEngine->pos_pressed.x - xpos) * 60.0f / (float)myEngine->swapChainExtent.width;
            myEngine->vertical_angle_offset = (myEngine->pos_pressed.y - ypos) * 60.0f / (float)myEngine->swapChainExtent.height;

            if (myEngine->horizatal_angle + myEngine->horizatal_angle_offset > 120.0f) {
                myEngine->horizatal_angle_offset = 120.0f - myEngine->horizatal_angle;
            }
            else if (myEngine->horizatal_angle + myEngine->horizatal_angle_offset < -120.0f) {
                myEngine->horizatal_angle_offset = -120.0f - myEngine->horizatal_angle;
            }


            if (myEngine->vertical_angle + myEngine->vertical_angle_offset > 60.0f) {
                myEngine->vertical_angle_offset = 60.0f - myEngine->vertical_angle;
            }
            else if (myEngine->vertical_angle + myEngine->vertical_angle_offset < -60.0f) {
                myEngine->vertical_angle_offset = -60.0f - myEngine->vertical_angle;
            }
    ...
    void cleanup() {
        ...
        for (size_t i = 0; i < LIGHTS_SIZE; ++i) {
            vkDestroyImageView(device, shadowMapDepthImageView[i], nullptr);
            vkDestroyImage(device, shadowMapDepthImage[i], nullptr);
            vkFreeMemory(device, shadowMapDepthImageMemory[i], nullptr);

            vkDestroyImageView(device, shadowMapColorImageView[i], nullptr);
            vkDestroyImage(device, shadowMapColorImage[i], nullptr);
            vkFreeMemory(device, shadowMapColorImageMemory[i], nullptr);
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            for (size_t j = 0; j < LIGHTS_SIZE; ++j) {
                vkDestroyBuffer(device, lightUniformBuffers[j][i], nullptr);
                vkFreeMemory(device, lightUniformBuffersMemory[j][i], nullptr);
            }
        ...
        for (size_t i = 0; i < LIGHTS_SIZE; ++i) {
            vkDestroyDescriptorPool(device, lightDescriptorPool[i], nullptr);
        }
        ...
        for (size_t i = 0; i < LIGHTS_SIZE; ++i) {
            vkDestroyCommandPool(device, shadowMapCommandPool[i], nullptr);
        }
    ...
    void createShadowMapFramebuffers() {
        shadowMapFramebuffer.resize(LIGHTS_SIZE);
        for (size_t i = 0; i < LIGHTS_SIZE; ++i) {
            std::array<VkImageView, 2> attachments = {
                shadowMapColorImageView[i],
                shadowMapDepthImageView[i],
            };
            ...
            if (vkCreateFramebuffer(device, &framebufferInfo, nullptr, &shadowMapFramebuffer[i]) != VK_SUCCESS) {
    ...
    void createShadowMapCommandPool() {
        shadowMapCommandPool.resize(LIGHTS_SIZE);
        for (size_t i = 0; i < LIGHTS_SIZE; ++i) {
            ...
            if (vkCreateCommandPool(device, &poolInfo, nullptr, &shadowMapCommandPool[i]) != VK_SUCCESS) {
    ...
    void createDescriptorSetLayout() {
        ...
        LightMVPLayoutBinding.descriptorCount = LIGHTS_SIZE;
        ...
        shadowMapSamplerLayoutBinding.descriptorCount = LIGHTS_SIZE;
    ...
    void createShadowMapColorResources() {
        ...
        shadowMapColorImage.resize(LIGHTS_SIZE);
        shadowMapColorImageView.resize(LIGHTS_SIZE);
        shadowMapColorImageMemory.resize(LIGHTS_SIZE);
        for (size_t i = 0; i < LIGHTS_SIZE; i++) {
            createImage(SHADOW_MAP_WIDTH, SHADOW_MAP_HEIGHT, 1, VK_SAMPLE_COUNT_1_BIT, colorFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, shadowMapColorImage[i], shadowMapColorImageMemory[i]);
            shadowMapColorImageView[i] = createImageView(shadowMapColorImage[i], colorFormat, VK_IMAGE_ASPECT_COLOR_BIT, 1);
        }
    ...
    void createShadowMapDepthResources() {
        ...
        shadowMapDepthImage.resize(LIGHTS_SIZE);
        shadowMapDepthImageView.resize(LIGHTS_SIZE);
        shadowMapDepthImageMemory.resize(LIGHTS_SIZE);
        for (size_t i = 0; i < LIGHTS_SIZE; i++) {
            createImage(SHADOW_MAP_WIDTH, SHADOW_MAP_HEIGHT, 1, VK_SAMPLE_COUNT_1_BIT, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, shadowMapDepthImage[i], shadowMapDepthImageMemory[i]);
            shadowMapDepthImageView[i] = createImageView(shadowMapDepthImage[i], depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT, 1);
        }
    }
    ...
    void loadModel() {
        ...
        std::array<glm::vec3, 6> floor = { glm::vec3(15.0f, -1.0f, -15.0f),
    ...
    void createShadowMapUniformBuffers() {
        lightUniformBuffers.resize(LIGHTS_SIZE);
        lightUniformBuffersMemory.resize(LIGHTS_SIZE);
        lightUniformBuffersMapped.resize(LIGHTS_SIZE);
        for (size_t i = 0; i < LIGHTS_SIZE; i++) {
            VkDeviceSize bufferSize = sizeof(UniformBufferObject);

            lightUniformBuffers[i].resize(MAX_FRAMES_IN_FLIGHT);
            lightUniformBuffersMemory[i].resize(MAX_FRAMES_IN_FLIGHT);
            lightUniformBuffersMapped[i].resize(MAX_FRAMES_IN_FLIGHT);

            for (size_t j = 0; j < MAX_FRAMES_IN_FLIGHT; j++) {
                createBuffer(bufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, lightUniformBuffers[i][j], lightUniformBuffersMemory[i][j]);

                vkMapMemory(device, lightUniformBuffersMemory[i][j], 0, bufferSize, 0, &lightUniformBuffersMapped[i][j]);
            }
        }
    ...
    void createShadowMapDescriptorPool() {
        lightDescriptorPool.resize(LIGHTS_SIZE);
        for (size_t i = 0; i < LIGHTS_SIZE; i++) {
            ...
            if (vkCreateDescriptorPool(device, &poolInfo, nullptr, &lightDescriptorPool[i]) != VK_SUCCESS) {
    ...
    void createShadowMapDescriptorSets() {
        lightDescriptorSets.resize(LIGHTS_SIZE);
        for (size_t i = 0; i < LIGHTS_SIZE; i++) {
            std::vector<VkDescriptorSetLayout> layouts(MAX_FRAMES_IN_FLIGHT, lightDescriptorSetLayout);
            VkDescriptorSetAllocateInfo allocInfo{};
            allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
            allocInfo.descriptorPool = lightDescriptorPool[i];
            allocInfo.descriptorSetCount = static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT);
            allocInfo.pSetLayouts = layouts.data();

            lightDescriptorSets[i].resize(MAX_FRAMES_IN_FLIGHT);
            if (vkAllocateDescriptorSets(device, &allocInfo, lightDescriptorSets[i].data()) != VK_SUCCESS) {
                throw std::runtime_error("failed to allocate descriptor sets!");
            }

            for (size_t j = 0; j < MAX_FRAMES_IN_FLIGHT; j++) {
                VkDescriptorBufferInfo bufferInfo{};
                bufferInfo.buffer = lightUniformBuffers[i][j];
                bufferInfo.offset = 0;
                ...
                descriptorWrites[0].dstSet = lightDescriptorSets[i][j];
    ...
    void createDescriptorPool() {
        ...
        poolSizes[2].descriptorCount = static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT) * LIGHTS_SIZE;
        poolSizes[3].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
        poolSizes[3].descriptorCount = static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT) * LIGHTS_SIZE;
    ...
    void createDescriptorSets() {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            ...
            VkDescriptorBufferInfo lightBufferInfo[LIGHTS_SIZE];
            for (uint32_t j = 0; j < LIGHTS_SIZE; j++)
            {
                lightBufferInfo[j].buffer = lightUniformBuffers[j][i];
                lightBufferInfo[j].offset = 0;
                lightBufferInfo[j].range = sizeof(UniformBufferObject);
            }
          
            VkDescriptorImageInfo shadowMapImageInfo[LIGHTS_SIZE];

            for (uint32_t j = 0; j < LIGHTS_SIZE; j++)
            {
                shadowMapImageInfo[j].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
                shadowMapImageInfo[j].imageView = shadowMapColorImageView[j];
                shadowMapImageInfo[j].sampler = shadowMapSampler;
            }
            ...
            descriptorWrites[2].descriptorCount = LIGHTS_SIZE;
            ...
            descriptorWrites[3].descriptorCount = LIGHTS_SIZE;
    ...
    void createShadowMapCommandBuffers() {
        shadowMapCommandBuffers.resize(LIGHTS_SIZE);
        for (size_t i = 0; i < LIGHTS_SIZE; ++i) {
            shadowMapCommandBuffers[i].resize(MAX_FRAMES_IN_FLIGHT);

            VkCommandBufferAllocateInfo allocInfo{};
            allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
            allocInfo.commandPool = shadowMapCommandPool[i];
            allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
            allocInfo.commandBufferCount = (uint32_t)shadowMapCommandBuffers[i].size();

            if (vkAllocateCommandBuffers(device, &allocInfo, shadowMapCommandBuffers[i].data()) != VK_SUCCESS) {
    ...
    void recordShadowMapCommandBuffer(VkCommandBuffer commandBuffer, VkFramebuffer frameBuffer,std::vector<VkDescriptorSet> descriptorSet) {
        VkCommandBufferBeginInfo beginInfo{};
        beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
        ...
        vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, shadowMapPipelineLayout, 0, 1, &descriptorSet[currentFrame], 0, nullptr);
    ...
    void updateShadowMapUniformBuffer(uint32_t currentImage) {
        ...
        std::array<LightUniformBufferObject, LIGHTS_SIZE> ubo{};
        ...
        ubo[0].zNear = 10.0f;
        ubo[0].zFar = 240.0f;
        ubo[0].light_position = { 15.0f,10.0f,20.0f };
        glm::mat4 view = glm::lookAt(ubo[0].light_position, glm::vec3(0.0f, 2.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
        glm::mat4 proj = clip * glm::perspective(glm::radians(60.0f), SHADOW_MAP_WIDTH / (float)SHADOW_MAP_HEIGHT, ubo[0].zNear, ubo[0].zFar);
        ubo[0].dir = { 0.0,0.0,0.0 };
        ubo[0].model = glm::rotate(glm::mat4(1.0f), 0.5f * time * glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
        ubo[0].model = glm::mat4(1.0f);
        ubo[0].view_proj = proj * view;
        ubo[0].light_emit = { 10.0f, 10.0f, 10.0f };
        ubo[0].perspective = true;
  

        memcpy(lightUniformBuffersMapped[0][currentImage], &ubo[0], sizeof(ubo[0]));
    }
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        ubo.model = glm::rotate(glm::mat4(1.0f), 0.25f * time * glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
        ...
        ubo.light_position = { 30.0f,40.0f,30.0f };
        ubo.light_emit = { 100.0f,100.0f,100.0f };
    ...
    void drawFrame() {
        ...
        vkResetFences(device, 1, &inFlightFences[currentFrame]);
            ...
            for (size_t i = 0; i < LIGHTS_SIZE; i++) {
                vkResetCommandBuffer(shadowMapCommandBuffers[i][currentFrame], /*VkCommandBufferResetFlagBits*/ 0);
                recordShadowMapCommandBuffer(shadowMapCommandBuffers[i][currentFrame], shadowMapFramebuffer[i], lightDescriptorSets[i]);
                ...
                shadowMapSubmitInfo.pCommandBuffers = &shadowMapCommandBuffers[i][currentFrame];
